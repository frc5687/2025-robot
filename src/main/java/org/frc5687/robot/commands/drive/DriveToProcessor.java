...
public class DriveToProcessor extends OutliersCommand {
    ...
    public DriveToProcessor(DriveSubsystem drive, DoubleSupplier ySupplier) {
        _drive = drive;
    ...
    public void initialize() {
        ...
        if (alliance.isPresent() && alliance.get() == Alliance.Red) {
            _lanePose = FlippingUtil.flipFieldPose(FieldConstants.Processor.processorLineUp);
        } else {
            _lanePose = FieldConstants.Processor.processorLineUp;
        }
    ...
    public void execute(double timestamp) {
        ...
        log("DriveToProcessor Target", targetPose, Pose2d.struct);

        Optional<Alliance> alliance = DriverStation.getAlliance();
        if (alliance.isPresent()) {
            if (alliance.get() == Alliance.Red) {
                _y = _ySupplier.getAsDouble() * -1;
            } else {
                _y = _ySupplier.getAsDouble();
            }
        }
        ...
        _drive.setDesiredChassisSpeeds(
                ChassisSpeeds.fromFieldRelativeSpeeds(
                        driveVelocity.getX(), _y, thetaVelocity, currentPose.getRotation()));
    }

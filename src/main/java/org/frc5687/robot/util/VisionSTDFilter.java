...
public class VisionSTDFilter {
    ...
    private static final double MIN_XY_STD = 0.005;
    ...
    private static final TunableDouble DISTANCE_SCALE_FACTOR =
            new TunableDouble("VisionSTDFilter", "DISTANCE_SCALE_FACTOR", 0.009);
    private static final TunableDouble AMBIGUITY_SCALE_FACTOR =
            new TunableDouble("VisionSTDFilter", "AMBIGUITY_SCALE_FACTOR", 0.03);
    private static final TunableDouble TIME_SCALE_FACTOR =
            new TunableDouble("VisionSTDFilter", "TIME_SCALE_FACTOR", 0.001);
    private static final TunableDouble VELOCITY_SCALE_FACTOR =
            new TunableDouble("VisionSTDFilter", "VELOCITY_SCALE_FACTOR", 0.3);
    private static final TunableDouble VELOCITY_EXPONENT =
            new TunableDouble("VisionSTDFilter", "VELOCITY_EXPONENT", 2.0);
    ...
    private CircularBuffer<EstimatedRobotPose> visionPoses = new CircularBuffer<>(20);
    ...
    public Matrix<N3, N1> calculateVisionStdDevs(
        ...
        visionPoses.addFirst(visionPose);
        // System.out.println("x stddev: "+getxstddev()+", y stddev: "+getystddev());
    ...
    private double calculateDistanceMultiplier(double minTagDistance) {
        return 0.05 + (minTagDistance * DISTANCE_SCALE_FACTOR.get());
    }

    private double calculateAmbiguityMultiplier(int numTags) {
        return 0.05 + (AMBIGUITY_SCALE_FACTOR.get() * (3.0 / Math.max(1, numTags)));
    }

    private double calculateTimeMultiplier(double currentTimestamp) {
        return 0.05 + ((currentTimestamp - _lastGoodPoseTimestamp) * TIME_SCALE_FACTOR.get());
    }

    private double calculateVelocityMultiplier() {
        return 0.05 + Math.pow(_robotVelocity * VELOCITY_SCALE_FACTOR.get(), VELOCITY_EXPONENT.get());
    }

    private double calculateQualityMultiplier(double tagQuality) {
        return 0.05 - Math.min(1.0, tagQuality);
    }

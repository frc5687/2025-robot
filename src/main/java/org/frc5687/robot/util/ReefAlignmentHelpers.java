...
public class ReefAlignmentHelpers {
    ...
    public static final TunableDouble QUICK_PICKUP_OFFSET =
            new TunableDouble("ReefAlignmentHelpers", "QUICK_PICKUP_OFFSET", 0.5);
    ...
    public static Pose2d calculateTargetPose(int faceNumber, ReefSide side) {
        ...
        double quickPickupOffset = (side == ReefSide.QUICK_PICK) ? QUICK_PICKUP_OFFSET.get() : 0;
        ...
        Translation2d quickPickMove =
                new Translation2d(
                        quickPickupOffset * Math.sin(lateralAngle),
                        quickPickupOffset * Math.cos(lateralAngle)); // This is not the right math

        Translation2d finalPosition = basePose.getTranslation().plus(lateralMove).plus(quickPickMove);
